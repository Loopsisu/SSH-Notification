...
# ==============================================================================
# SSH Login Notifier for Bark - Installation Script (Enhanced Version)
# Author: Your Name
# GitHub: https://github.com/your-username/your-repo
# Description: This version allows setting a custom Bark server URL.
# ==============================================================================
...
# 4. (新增) 处理 Bark URL，提供默认值并确保格式正确
if [ -z "$BARK_URL" ]; then
  BARK_URL="https://api.day.app/"
  echo -e "${YELLOW}您未提供 URL，将使用官方 Bark 服务器: ${NC}${BARK_URL}"
else
  # 健壮性检查：如果用户输入的 URL 不以 / 结尾，则自动为其添加
  if [[ "${BARK_URL}" != */ ]]; then
    BARK_URL="${BARK_URL}/"
  fi
...
# 5. 创建推送脚本
# 使用 here document 创建脚本文件，并动态插入用户提供的 BARK_KEY 和 BARK_URL
# 注意：$PAM_USER 等变量需要转义，以便它们在最终脚本中保持原样
tee "$NOTIFY_SCRIPT_PATH" > /dev/null <<EOF
#!/bin/bash
# This script is automatically generated by install.sh
...
if grep -q "pam_exec.so.*${NOTIFY_SCRIPT_PATH}" "${PAM_SSHD_CONFIG}"; then
  ...
  # 创建备份，更安全的备份文件名
  cp "${PAM_SSHD_CONFIG}" "${PAM_SSHD_CONFIG}.bak.$(date +%F_%T)"
  echo "PAM 配置文件备份已创建。"
  # 在文件末尾添加配置
  echo "" >> "${PAM_SSHD_CONFIG}"
  echo "# SSH Login Notifier for Bark - Added by install.sh" >> "${PAM_SSHD_CONFIG}"
  echo "session    optional     pam_exec.so quiet ${NOTIFY_SCRIPT_PATH}" >> "${PAM_SSHD_CONFIG}"
